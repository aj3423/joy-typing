...
**For those who wants to:**
 - Do squat while programming or just avoid maintaining the typing posture.
 - Play any PC game with motion control.
...
## How it works
** 1. The Mode concept**
 The idea comes from Vim, there could be different kinds of modes: *normal*, *speech*, *motion control*. The Joy-Con has a very limited number of buttons, but a same button can trigger different actions in different modes, hence it's greatly extended.

**  2. Word Mapping**
 Words can be mapped to different actions like:
 - Replacement
  e.g. `dash` -> `-`, `twenty twenty two` -> `2022`
 - Decoration
 e.g. `snake hello world` -> `hello_world`, `camel hello world` -> `helloWorld`
 - Hotkey
 e.g. `control alt delete` -> `task explorer launched`
 - Run shell script
 e.g. `launch browser` -> `brave-browser --no-sandbox www.test.com`
...
- Hold the sync button for 2 seconds, the lights start blinking.
TODO image here
...
TODO binary release page link


## Build from Source
1. install dependency `hidapi`
 - Windows
 Download the release file from https://github.com/libusb/hidapi/releases, which contains header, lib and dll for compiling.
 - Linux
 `apt install libhidapi-dev`
2. Install golang from https://go.dev/dl/
3. Clone this repo: TODO `git clone github.com/aj3423/joy-typing`
...
`run calc.exe` `delay 1 second` `type "1+1"` `press enter`
Which is executed by 4 executors: `shell``delay``typing``hotkey``
...
| [hotkey]     | trigger a hotkey  | key array</br> [key list](https://github.com/go-vgo/robotgo/blob/master/key.go#L205)</br>e.g. `launch terminal -> [hotkey] t control alt` |
